...
import { AlertTriangle, CheckCircle2, TrendingUp, BarChart3, Truck } from "lucide-react";
...
interface TotalDeliveryTimeInsightsSectionProps {
...
export const TotalDeliveryTimeInsightsSection = ({ stats }: TotalDeliveryTimeInsightsSectionProps) => {
  if (stats.length === 0) return null;

  // Find restaurant with best performance (shortest total time)
  const sortedBySpeed = [...stats].sort((a, b) => a.avgTotalTime - b.avgTotalTime);
  ...
  const gap = worstPerformer.avgTotalTime - bestPerformer.avgTotalTime;
  
  // Calculate network average
  const totalWeighted = stats.reduce((sum, s) => sum + s.avgTotalTime * s.orderCount, 0);
  ...
  const fastRestaurants = stats.filter(s => s.avgTotalTime <= 30);
  ...
  const insights = [
    // Best performer
    {
      ...
      detail: `${formatMinutesToDisplay(bestPerformer.avgTotalTime)} en moyenne`,
    },
    ...
    worstPerformer.id !== bestPerformer.id && {
      ...
      detail: `${formatMinutesToDisplay(worstPerformer.avgTotalTime)} en moyenne`,
    },
    // Network average
    networkAverage > 0 && {
      icon: BarChart3,
