...
interface PrepTimeHeatmapGridProps {
...
const getIntensityColor = (avgTime: number): string => {
  ...
  if (avgTime <= 4) return "bg-emerald-500/60";
  if (avgTime <= 5) return "bg-emerald-400/50";
  if (avgTime <= 6) return "bg-amber-400/50";
  if (avgTime <= 8) return "bg-orange-400/60";
  return "bg-red-500/70";
};

export const PrepTimeHeatmapGrid = ({ stats, dateRange }: PrepTimeHeatmapGridProps) => {
  ...
  // Per-restaurant weekly view with average prep time per day
    ...
    <div className="space-y-6">
      ...
      <div>
        <h4 className="text-sm font-medium mb-3 text-muted-foreground">
          Temps de préparation moyen par jour de la semaine
        </h4>
