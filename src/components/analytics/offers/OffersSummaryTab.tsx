...
import type { OffersAnalyticsResult, RestaurantOfferStats } from "@/hooks/useOffersAnalytics";
...
export function OffersSummaryTab({ data, restaurants }: Props) {
  const { kpis, restaurantStats, monthlyStats, heatmapData, anomalies } = data;
  ...
  const chartKeys = useMemo(() => {
    const top = [...restaurantStats].sort((a, b) => b.totalFees - a.totalFees).slice(0, 8);
    const keys = top.map((r) => r.restaurantName);
    keys.push("Autres");
    return keys;
  }, [restaurantStats]);
          ...
          <div className="overflow-auto max-h-[500px]">
            <Table>
              ...
              <TableBody>
                ...
                {sortedStats.map((rs: RestaurantOfferStats) => (
