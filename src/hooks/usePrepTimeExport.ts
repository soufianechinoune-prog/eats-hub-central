...
interface PrepTimeExportData {
  periodLabel: string;
  globalStats: {
    avgPrepTime: number;
    totalOrders: number;
    fastRestaurants: number; // < 5 min
    slowRestaurants: number; // > 8 min
  ...
  restaurants: RestaurantPrepTime[];
}
...
const getStatusLabel = (prepTime: number): string => {
  if (prepTime <= 4) return "Excellent";
  if (prepTime <= 5) return "Tres bien";
  if (prepTime <= 6) return "Bon";
  if (prepTime <= 8) return "A surveiller";
  return "Lent";
};

const getStatusColor = (prepTime: number): [number, number, number] => {
  if (prepTime <= 4) return [16, 185, 129]; // emerald-500
  if (prepTime <= 5) return [34, 197, 94]; // green-500
  if (prepTime <= 6) return [245, 158, 11]; // amber-500
  if (prepTime <= 8) return [249, 115, 22]; // orange-500
...
export const usePrepTimeExport = () => {
  const [isExporting, setIsExporting] = useState(false);

  const exportToPDF = async (data: PrepTimeExportData) => {
    setIsExporting(true);
    ...
    try {
      ...
      pdf.text("Rapport Temps de Preparation", margin, 35);
      ...
      drawKPICard(margin, yPos, "Temps moyen reseau", formatMinutesToDisplay(data.globalStats.avgPrepTime));
      drawKPICard(margin + kpiCardWidth + 10, yPos, "Total commandes", formatNumber(data.globalStats.totalOrders));

      yPos += kpiCardHeight + 10;

      drawKPICard(margin, yPos, "Restaurants rapides (< 5min)", `${data.globalStats.fastRestaurants} restaurants`);
      drawKPICard(margin + kpiCardWidth + 10, yPos, "Restaurants lents (> 8min)", `${data.globalStats.slowRestaurants} restaurants`);
      ...
      data.distribution.forEach((item) => {
        ...
        else if (item.label.includes("Tres")) barColor = [34, 197, 94];
      ...
      const drawMiniTable = (title: string, items: RestaurantPrepTime[], startY: number, isTop: boolean) => {
        ...
        items.forEach((item) => {
          ...
          pdf.text(formatMinutesToDisplay(item.avgPrepTime), margin + contentWidth - 50, itemY);
          pdf.setTextColor(...mutedColor);
      ...
      const drawPageHeader = () => {
        ...
        pdf.setFontSize(12);
        pdf.setFont("helvetica", "bold");
        pdf.text("Rapport Temps de Preparation", margin, 13);
      ...
      for (let pageIdx = 0; pageIdx < totalDetailPages; pageIdx++) {
        ...
        pageData.forEach((item, idx) => {
          ...
          pdf.setFont("helvetica", "bold");
          pdf.text(formatMinutesToDisplay(item.avgPrepTime), margin + 115, yPos + 5.5);
          
          pdf.setFont("helvetica", "normal");
          pdf.text(item.orderCount.toString(), margin + 140, yPos + 5.5);

          // Status with color
          const [r, g, b] = getStatusColor(item.avgPrepTime);
          pdf.setTextColor(r, g, b);
          pdf.text(getStatusLabel(item.avgPrepTime), margin + 165, yPos + 5.5);
      ...
      // Save PDF
      const fileName = `rapport-temps-preparation-${format(new Date(), "yyyy-MM-dd")}.pdf`;
      pdf.save(fileName);
