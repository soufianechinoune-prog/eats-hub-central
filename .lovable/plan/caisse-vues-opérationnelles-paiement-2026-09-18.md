...
## Détails techniques
- Tables `splash_tickets` / `splash_ticket_lines` : clé naturelle **(restaurant_id, station/caisse, ticket_id Splash)** — le relevé de format vérifie explicitement, sur un Tasty Crousty multi-caisses, si l'identifiant de ticket est unique par restaurant ou seulement par caisse ; si par caisse, la station reste dans la clé (sinon deux caisses écraseraient silencieusement leurs tickets). Contrôle de non-perte après import : nombre de tickets importés vs nombre renvoyé par l'API. `chain_id` porté sur les deux tables, triggers de cohérence de marque comme sur les tables Splash existantes, GRANT `authenticated` + `service_role`, RLS via `user_has_chain_access`, `anon` révoqué. Index (restaurant_id, ticket_datetime) et (ticket_id) pour les lignes.
- Table `splash_restaurant_credentials` (`restaurant_id`, `chain_id`, `splash_restaurant_id`, `client_id`, `client_secret_encrypted`, `is_active`, `last_ok_at`, `last_error`) : RLS activée, GRANT `service_role` uniquement, aucun GRANT `anon`/`authenticated`, secret chiffré via `supabase_vault`, jamais renvoyé par une RPC ni journalisé. Import initial des deux fichiers Excel via edge function admin protégée (super-admin), résolution `splash_restaurant_id` → `restaurant_id` par `splash360_restaurant_mapping`.
- Edge function `splash-orders-sync` : OAuth `client_credentials` sur `api2.splash360.fr`, credentials lus dans `splash_restaurant_credentials`, pagination `pageSize=100`, upsert idempotent.
- File de rattrapage `splash_ticket_backfill_jobs` (restaurant + mois, statut, tentatives, `next_attempt_at`) consommée par un worker cron en priorité basse : pacing type Uber (`vague`), ~2 jobs/tick, délai entre pages, back-off sur 429/5xx, reprise là où le job s'est arrêté. Enfilage manuel et progressif (témoin 1 mois, puis vagues par restaurant), jamais les 182 d'un coup.
