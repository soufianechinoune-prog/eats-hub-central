...
## Objectif
Compléter la migration RLS en ajoutant les DROP manquants pour `restaurants`, `chains` et `payout_adjustments`.

## Correction
Ajouter ces DROP au début de la migration, avant les CREATE POLICY :

```sql
-- restaurants (policies publiques restantes)
...
-- chains (policies publiques restantes)
...
-- payout_adjustments (policies publiques restantes)
...
Ces lignes s'ajoutent aux DROP déjà prévus dans la migration précédente. La migration complète reste identique pour les 21 tables, avec ces 12 DROP supplémentaires en tête.

## Migration SQL finale complète
...
## Impact
- Toutes les anciennes policies permissives sont supprimées (y compris celles identifiées par l'ingénieure)
- Une seule policy propre par table : `FOR ALL TO authenticated USING (true) WITH CHECK (true)`
- `weather_data` et `import_guide_screenshots` non touchées (publiques intentionnellement)
- Les Edge Functions avec `service_role_key` ne sont pas affectées (bypass RLS)
