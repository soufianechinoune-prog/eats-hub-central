DROP FUNCTION IF EXISTS public.get_orders_finance_detail(integer, integer, uuid[]);
...
AS $function$
  SELECT
    ...
    COALESCE(SUM(CASE WHEN o.refund_incl_vat < 0 THEN ABS(o.refund_incl_vat) ELSE 0 END), 0) AS refund_to_customer,
    COALESCE(SUM(CASE WHEN o.refund_incl_vat > 0 THEN o.refund_incl_vat ELSE 0 END), 0) AS refund_uber_cancellation,
    COALESCE(SUM(CASE WHEN o.refund_incl_vat < 0 THEN ABS(o.refund_incl_vat) ELSE 0 END), 0)
      - COALESCE(SUM(CASE WHEN o.refund_incl_vat > 0 THEN o.refund_incl_vat ELSE 0 END), 0) AS refund_net
