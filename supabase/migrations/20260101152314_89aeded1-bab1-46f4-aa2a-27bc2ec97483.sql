-- Drop and recreate get_monthly_payouts_detail with additional HT/TVA columns for detailed breakdown
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(p.sales_excl_vat, 0) as sales_excl_vat,
    COALESCE(p.refund_incl_vat, 0) as refund_incl_vat,
    COALESCE(p.refund_excl_vat, 0) as refund_excl_vat,
    COALESCE(p.vat_refund, 0) as vat_refund,
    COALESCE(p.item_promo_incl_vat, 0) as item_promo_incl_vat,
    COALESCE(p.item_promo_excl_vat, 0) as item_promo_excl_vat,
    COALESCE(p.uber_fee_after_promo_incl_vat, 0) as uber_fee_after_promo_incl_vat,
    COALESCE(p.uber_fee_after_promo_excl_vat, 0) as uber_fee_after_promo_excl_vat,
    ...
    COALESCE(p.delivery_promo_excl_vat, 0) as delivery_promo_excl_vat,
    COALESCE(p.price_adjustment_incl_vat, 0) as price_adjustment_incl_vat,
    COALESCE(p.price_adjustment_excl_vat, 0) as price_adjustment_excl_vat,
