CREATE OR REPLACE FUNCTION public.enqueue_order_history_backfill(p_restaurant_id uuid, p_months date[])
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_resto record;
  v_month date;
  v_min_date date := (current_date - interval '188 days')::date;
BEGIN
  ...
  FOREACH v_month IN ARRAY p_months LOOP
    -- STRICT: le DÉBUT du mois doit être dans la fenêtre 188j sinon Uber rejette.
    -- Pour ces mois, le CSV couvre déjà la donnée → skip silencieux.
    IF date_trunc('month', v_month)::date < v_min_date THEN
      CONTINUE;
    END IF;
