...
BEGIN
  ...
  WITH unified AS (
    ...
    SELECT
      ...
      (o.uber_one_status ILIKE 'Membre%') AS is_uber_one,
  ...
  SELECT
    CASE
      WHEN p_granularity = 'daily' THEN to_char((u.order_datetime AT TIME ZONE 'Europe/Paris')::date, 'YYYY-MM-DD')
      ELSE to_char((u.order_datetime AT TIME ZONE 'Europe/Paris')::date, 'YYYY-MM')
    END AS period_key,
    u.restaurant_id,
