...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE refund_contested_incl_vat > 0 OR refund_incl_vat > 0)::bigint AS contested_won_count,
    ROUND(COALESCE(SUM(
    ...
    COUNT(*) FILTER (WHERE refund_incl_vat < 0 OR refund_contested_incl_vat > 0 OR refund_incl_vat > 0)::bigint AS net_count,
    ROUND(COALESCE(SUM(
      CASE WHEN refund_incl_vat < 0 THEN refund_incl_vat ELSE 0 END
      + GREATEST(refund_contested_incl_vat, 0)
      + CASE WHEN refund_incl_vat > 0 THEN refund_incl_vat ELSE 0 END
