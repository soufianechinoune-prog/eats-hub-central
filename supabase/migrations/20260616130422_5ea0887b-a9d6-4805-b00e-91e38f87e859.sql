...
CREATE INDEX IF NOT EXISTS idx_backfill_jobs_pending_next_attempt
  ON public.backfill_jobs (next_attempt_at, vague, month_start)
...
BEGIN
  ...
  WITH ranked AS (
    ...
    WHERE bj.status = 'pending'
      AND (bj.next_attempt_at IS NULL OR bj.next_attempt_at <= now())
  ),
  picked AS (
    SELECT r.bj_id
    FROM ranked r
