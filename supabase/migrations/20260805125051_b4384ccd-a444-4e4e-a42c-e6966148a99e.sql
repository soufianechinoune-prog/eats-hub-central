CREATE OR REPLACE FUNCTION public.get_splash_onsite_monthly_v2(
  p_chain_id uuid,
...
BEGIN
  ...
  monthly AS (
    SELECT
      daily.rid,
      r.name::text AS rname,
      EXTRACT(YEAR FROM daily.d)::int AS y,
      EXTRACT(MONTH FROM daily.d)::int AS m,
      SUM(daily.onsite_ttc)::numeric AS ttc,
      SUM(daily.onsite_ht)::numeric AS ht,
      SUM(daily.onsite_orders)::int AS orders,
    ...
    SELECT
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'days_zero', monthly.days_zero
        ) ORDER BY monthly.y, monthly.m
      ) AS months
    FROM monthly
    GROUP BY monthly.rid, monthly.rname
  ),
  unmapped AS (
    SELECT
      COUNT(DISTINCT s.restaurant_splash_id)::int AS ids,
  ...
  SELECT jsonb_build_object(
    ...
    'restaurants', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          'restaurant_id', per_resto.rid,
          'name', per_resto.rname,
          'months', per_resto.months
        ) ORDER BY per_resto.rname
      ) FROM per_resto
    ), '[]'::jsonb),
  ...
  RETURN COALESCE(v_result, jsonb_build_object('year', v_year, 'restaurants', '[]'::jsonb, 'coverage', jsonb_build_object('days_zero_current', 0, 'unmapped_splash_ids', 0, 'unmapped_revenue_ttc', 0)));
END;
$function$;

GRANT EXECUTE ON FUNCTION public.get_splash_onsite_monthly_v2(uuid, uuid[], integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_splash_onsite_monthly_v2(uuid, uuid[], integer) TO service_role;