-- Drop the old global benchmark function
DROP FUNCTION IF EXISTS public.get_local_benchmark_conversion(uuid, date, date);

-- Helper: normalize city name (lowercase, no accents, strip arrondissement suffixes)
...
AS $$
  SELECT regexp_replace(
    regexp_replace(
      ...
      '\s+\d+\s*(er|eme|ème|e)?\s*$', '', 'g'
    ),
    '[^a-z0-9]', '', 'g'
  );
$$;

-- New contextual benchmark function: given a restaurant, return the average
-- of competitors (other chains) in the same city, with postal_code fallback.
...
BEGIN
  ...
  -- Step 1: Try matching by normalized city
  WITH competitors AS (
    SELECT r.id
    FROM public.restaurants r
  ...
  -- Step 2: Fallback to postal_code
