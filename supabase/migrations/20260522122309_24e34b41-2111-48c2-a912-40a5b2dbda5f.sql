...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(CASE WHEN ord.refund_incl_vat > 0 THEN ord.refund_incl_vat ELSE 0 END), 0) AS refund_uber_cancellation,
    COALESCE(SUM(CASE WHEN ord.refund_incl_vat < 0 THEN ABS(ord.refund_incl_vat) ELSE 0 END), 0)
      - COALESCE(SUM(CASE WHEN ord.refund_incl_vat > 0 THEN ord.refund_incl_vat ELSE 0 END), 0) AS refund_net
