CREATE INDEX IF NOT EXISTS idx_splash360_daily_sales_cash_scope
  ON public.splash360_daily_sales (chain_id, restaurant_id, date, platform)
  WHERE granularity = 'day' AND restaurant_splash_id <> 0;
...
AS $$
  ...
  scoped_restaurants AS MATERIALIZED (
    SELECT unnest(p_restaurant_ids) AS id
    WHERE p_chain_id IS NULL
      AND p_restaurant_ids IS NOT NULL
      AND array_length(p_restaurant_ids, 1) > 0
  ),
  cur AS (
    SELECT s.date, s.platform,
           SUM(COALESCE(s.revenue_ttc, 0)) AS ttc,
           SUM(COALESCE(s.revenue_ht, 0)) AS ht,
           SUM(COALESCE(s.vat_amount, 0)) AS vat,
           SUM(COALESCE(s.order_count, 0)) AS orders
    FROM public.splash360_daily_sales s
    JOIN accessible_chains ac ON ac.id = s.chain_id
    WHERE s.granularity = 'day'
      AND s.restaurant_splash_id <> 0
      AND s.date BETWEEN p_start_date AND p_end_date
      AND (
        p_chain_id IS NOT NULL
        OR s.restaurant_id IN (SELECT id FROM scoped_restaurants)
      )
    GROUP BY s.date, s.platform
  ),
  prev AS (
    SELECT s.date, s.platform,
           SUM(COALESCE(s.revenue_ttc, 0)) AS ttc,
