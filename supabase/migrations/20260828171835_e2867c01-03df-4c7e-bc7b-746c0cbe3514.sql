DROP FUNCTION IF EXISTS public.get_delivery_pnl(uuid, date, date, uuid[]);
...
REVOKE ALL ON FUNCTION public.get_delivery_pnl(date,date,uuid[]) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_delivery_pnl(date,date,uuid[]) TO authenticated;