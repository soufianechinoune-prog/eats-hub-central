...
AS $$
  ...
  WHERE o.restaurant_id = ANY(p_restaurant_ids)
    ...
    AND (o.order_datetime AT TIME ZONE 'Europe/Paris')::date >= p_date_from
    AND (o.order_datetime AT TIME ZONE 'Europe/Paris')::date <= p_date_to
  GROUP BY o.restaurant_id, o.meal_voucher_provider;
$$;

GRANT EXECUTE ON FUNCTION public.get_meal_voucher_breakdown(uuid[], date, date) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.get_meal_voucher_totals(uuid[], date, date) TO authenticated;
