-- Supprimer l'ancienne fonction et la recréer avec les nouveaux champs
DROP FUNCTION IF EXISTS public.get_monthly_payouts_detail(integer, integer, uuid[]);

CREATE FUNCTION public.get_monthly_payouts_detail(p_year integer, p_month integer, p_restaurant_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS TABLE(
   restaurant_id uuid, 
   payout_date date, 
