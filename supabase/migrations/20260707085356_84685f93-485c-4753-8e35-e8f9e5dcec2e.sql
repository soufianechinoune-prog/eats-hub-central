...
BEGIN
  ...
  network_total AS (
    SELECT
      ...
      COALESCE(SUM(ca_brut_ttc - fee_ttc), 0) AS ca_net_ttc,
      COALESCE(SUM(ca_brut_ht - fee_ht), 0) AS ca_net_ht,
      COALESCE(SUM(fee_ttc), 0) AS commission_uber,
      COALESCE(SUM(marketing_fee), 0) AS marketing_fee,
      COALESCE(SUM(service_fee), 0) AS service_fee,
      COALESCE(SUM(payout + meal_voucher), 0) AS payout_total
    FROM base
  ),
  by_day AS (
    SELECT
